    ...
    def get_path(self, start_idx, goal_idx):
        ...
        while open_list.not_empty():
            ...
            for nb_cell in self.get_free_neighbors(cell):
                tent_g_cost = cell.g_cost + Distance.from_separation(cell.idx, nb_cell.idx)
                #print("bp3")
                #print (cell)
...
def gen_mask(cell_size, radius):
    ...
    """
    index_masking = int64(radius/cell_size)
    
    indices = []
    
    for i in range(-index_masking,index_masking+1):
        for j in range(-index_masking,index_masking+1):
            indices.append([i,j])
    
    return indices

# ================================ BEGIN ===========================================
MotionModel = lab1.OdometryMM
LOS = lab1.LOS
#LOS = lab2_aux.GeneralLOS
inverse_sensor_model = lab1.inverse_sensor_model
