...
def main():
    ...
    while isnan(rbt_scan[0]) or isnan(rbt_true[0]):
        pass
    
    # Data structures
    occ_grid = OccupancyGrid((-5,-5), (5,5), 0.1, 0.5) # OccupancyGrid((-5,-5), (5,5), ???, ???)
    los = LOS(occ_grid)
    motion_model = None #??? # OdometryMM((0,0,0), (0,0), 0.16, 0.066), # VelocityMM((0,0,0), 0.16, 0.066)
...
#        continue
        ...
        if (rospy.get_time() > t): # every 50 ms
            ...
            rbt_pos = rbt_true #??? motion_model.calculate(???) # rbt_wheels, rbt_control
            
            
            #f = open("src/ee4308_Lab1/scripts/inverse_sensor_model.txt", "w")
            ...
            for i in xrange(360):
                ...
                if scan[i] != inf: # range reading is < max range ==> occupied
                    ...
                    occ_grid.set_cell_pos(end_pos, bayes_calc(0.4, occ_grid.get_cell_pos(end_pos)))
                else: # range reading is inf ==> no obstacle found
                    end_pos = inverse_sensor_model(MAX_RNG, i, rbt_pos)
                    # # set the last cell as free
                    occ_grid.set_cell_pos(end_pos, bayes_calc(-0.2, occ_grid.get_cell_pos(end_pos)))
                # # set all cells between current cell and last cell as free
                for idx in los.calculate(rbt_pos, end_pos):
                    occ_grid.set_cell_idx(idx, bayes_calc(-0.2, occ_grid.get_cell_idx(idx))) #??? # occ_grid.set_cell_idx(idx, ???)
